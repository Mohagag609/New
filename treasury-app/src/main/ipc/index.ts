...
export function registerIpcHandlers() {
  ...
  ipcMain.handle('accounts:create', (event, data) => {
  ...
  ipcMain.handle('accounts:update', (event, id, data) => {
  ...
  ipcMain.handle('accounts:delete', (event, id) => deleteAccount(id));

  // Party Handlers
  ipcMain.handle('parties:list', () => listParties());

  ipcMain.handle('parties:create', (event, data) => {
  ...
  ipcMain.handle('parties:update', (event, id, data) => {
  ...
  ipcMain.handle('partners:delete', (event, id) => deletePartner(id));

  // Expense Category Handlers
  ipcMain.handle('expenses:categories:list', () => listExpenseCategories());

  ipcMain.handle('expenses:categories:create', (event, data) => {
  ...
  ipcMain.handle('expenses:categories:update', (event, id, data) => {
    const validation = UpdateExpenseCategorySchema.safeParse(data);
    if (!validation.success) throw new Error(`Invalid expense category data: ${validation.error.message}`);
    return updateExpenseCategory(id, validation.data);
  });

  ipcMain.handle('expenses:categories:delete', (event, id) => deleteExpenseCategory(id));

  // Voucher Handlers
  ipcMain.handle('vouchers:list', () => listVouchers());

  ipcMain.handle('vouchers:create', (event, data) => {
  ...
  ipcMain.handle('vouchers:update', (event, id, data) => {
    const validation = UpdateVoucherSchema.safeParse(data);
    if (!validation.success) throw new Error(`Invalid voucher data: ${validation.error.message}`);
    return updateVoucher(id, validation.data);
  });

  ipcMain.handle('vouchers:delete', (event, id) => deleteVoucher(id));

  // Dashboard Handlers
  ipcMain.handle('dashboard:kpis', (event, range) => getKpis(range));
  ipcMain.handle('dashboard:chart', (event, range) => getMonthlyChartData(range));

  // Reports Handlers
  ipcMain.handle('reports:account-balances', () => getAccountBalances());
  ipcMain.handle('reports:income-statement', (event, range) => getIncomeStatement(range));

  // Import Handlers
  ipcMain.handle('import:select-file', (event, title, filters) => handleSelectFile(title, filters));
  ipcMain.handle('import:run', (event, mappingPath, dataPath) => handleRunImport(mappingPath, dataPath));


  // ... register other handlers here in the future
